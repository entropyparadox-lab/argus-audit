...
mod tests {
    ...
    fn test_resolve_ssh_key_with_auth_info_and_authorized_keys() {
        ...
        let auth_keys_content = format!("ssh-ed25519 {} charles@cycorld.com\n", pub_blob);
        ...
        assert_eq!(comment.as_deref(), Some("charles@cycorld.com"));
    }
    ...
    fn test_resolve_single_key_fallback() {
        ...
        let pub_blob = "AAAAC3NzaC1lZDI1NTE5AAAAIAGP15qR4B0l5w/Fz5G4YhXhV7zX7Fz1Z9tT8rG9p1bA";
        let auth_keys_content = format!("ssh-ed25519 {} vodana@ep-mac\n", pub_blob);
        fs::write(ssh_dir.join("authorized_keys"), auth_keys_content).unwrap();
        ...
        assert!(fp.is_some());
        assert_eq!(comment.as_deref(), Some("vodana@ep-mac"));
    }
}
