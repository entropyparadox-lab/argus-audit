...
## 1. Central Collector Setup (This Server: `collector-01-livemixer`)
...
3. **Run installation on target server**:
   ...
   sudo ./deploy/scripts/setup-agent.sh http://audit-collector.internal:19532
   ```
...
2. **Add auto-wrap hook to `~/.zshrc`**:
   ...
   if [[ -z "$ARGUS_ACTIVE" && -t 0 ]]; then
       ...
       exec /usr/local/bin/argus-agent wrap --collector "http://audit-collector.internal:19532"
   fi
   ```
...
argus --collector http://audit-collector.internal:19532 sessions
```
